...
class unet(nn.Module):
    def __init__(self, feature_scale=4,  # to reduce dimensionality
        ...
        if from_latent_hidden_layers:
            setattr(self, 'conv_' + str(num_encoding_layers) + '_stage' + str(ns), nn.Sequential(
                unetConv2(self.filters[num_encoding_layers - 2], self.filters[num_encoding_layers - 1],
                          self.is_batchnorm, padding=1),
                nn.MaxPool2d(kernel_size=2)
                ))
        else:
            setattr(self, 'conv_' + str(num_encoding_layers) + '_stage' + str(ns),
                    unetConv2(self.filters[num_encoding_layers - 2], self.filters[num_encoding_layers - 1],
                              self.is_batchnorm, padding=1))
        ...
        ############ background ###############
        if skip_background:
            setattr(self, 'conv_1_stage_bg' + str(ns),
                    unetConv2(self.in_channels, self.filters[0], self.is_batchnorm, padding=1))
        ...
        # FIXME: fix hardcode
        self.to_pose = MLP.MLP_fromLatent(d_in=256, d_hidden=2048, d_out=51, n_hidden=n_hidden_to3Dpose, dropout=0.5)
        ...
        if self.skip_connections:
            for li in range(1, num_encoding_layers - 1):
                setattr(self, 'upconv_' + str(li) + '_stage' + str(ns),
                        unetUp(self.filters[num_encoding_layers - li], self.filters[num_encoding_layers - li - 1],
                               upper_conv, padding=1))
                # setattr(self, 'upconv_2_stage' + str(ns), unetUp(self.filters[2], self.filters[1], upper_conv, padding=1))
        else:
            for li in range(1, num_encoding_layers - 1):
                setattr(self, 'upconv_' + str(li) + '_stage' + str(ns),
                        unetUpNoSKip(self.filters[num_encoding_layers - li], self.filters[num_encoding_layers - li - 1],
                                     upper_conv, padding=1))
        ...
        if self.skip_connections or self.skip_background:
            setattr(self, 'upconv_' + str(num_encoding_layers - 1) + '_stage' + str(ns),
    ...
    def forward(self, input_dict):
        ...
        def shuffle_segment(list, start, end):
            ...
            if self.training:
                if 0 and end - start == 2:  # Note, was not enabled in ECCV submission, diabled now too HACK
                    ...
                    if prob[
                        0] > 1 / self.num_cameras:  # assuming four cameras, make it more often that one of the others is taken, rather than just autoencoding (no flip, which would happen 50% otherwise)
        ...
        if self.skip_connections:
            ...
            out_deconv = output.view(batch_size, map_channels, 1, 1).expand(batch_size, map_channels, map_width,
                                                                            map_width)
            for li in range(1, self.num_encoding_layers - 1):
                out_deconv = getattr(self, 'upconv_' + str(li) + '_stage' + str(ns))(out_deconv)

            if self.skip_background:
                out_deconv = getattr(self, 'upconv_' + str(self.num_encoding_layers - 1) + '_stage' + str(ns))(
                    conv1_bg_shuffled, out_deconv)
            else:
                out_deconv = getattr(self, 'upconv_' + str(self.num_encoding_layers - 1) + '_stage' + str(ns))(
                    out_deconv)
        ...
        output_pose = self.to_pose.forward({'latent_3d': output})['3D']
